...
A simple program that generates an image of a QR code. The code can then be transferred from a smartphone (which can run out of power) to an epaper key-fob (that does not have a battery, so will never run out of power).
